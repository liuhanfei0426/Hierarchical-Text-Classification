...
# learning_rates = [3e-5]
# logits_values = [0.3]
learning_rates = [5e-6, 1e-5, 2e-5, 3e-5, 4e-5, 5e-5]
logits_values = [0.3, 0.5]
train_batch_size = 16
    ...
    with redirect_stdout(f):
        for logits_value in logits_values:
            for learning_rate in learning_rates:
                best_model_path = f"best_19_logits={logits_value}_lr={learning_rate}.pt"
                print("*************************************************************************")
                ...
                for batch in val_loader:
                    ...
                    with torch.no_grad():
                        ...
                        # print("logits_np: ", logits_np)

                        # 现在的操作
                        logits_sigmoid = torch.sigmoid(logits)
                        logits_np_sigmoid = logits_sigmoid.cpu().numpy()
                        # print("eval_logits_sigmoid: ", logits_np_sigmoid)

                        predictions = np.where(logits_np_sigmoid >= logits_value, 1, 0)
                ...
                if metrics['micro']['f1'] > all_best_f1_micro:
                    ...
                    # 保存预测的结果
                    np.savetxt('predict_19.tsv', predict.astype(int), delimiter='\t', fmt='%d')
                    np.savetxt('gt_19.tsv', gt.astype(int), delimiter='\t', fmt='%d')
                ...
                print("Weighted: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    metrics['weighted']['precision'] * 100,
                ...
                big_per_class_result = metric_big_per_class(gt, predict)
                ...
                print(f"\n4个大类的总体结果: \n")
                print("Micro: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    big_class_result['micro']['precision'] * 100,
                    big_class_result['micro']['recall'] * 100,
                    big_class_result['micro']['f1'] * 100))
                print("Macro: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    big_class_result['macro']['precision'] * 100,
                    big_class_result['macro']['recall'] * 100,
                    big_class_result['macro']['f1'] * 100))
                print("Weighted: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    big_class_result['weighted']['precision'] * 100,
                    big_class_result['weighted']['recall'] * 100,
                    big_class_result['weighted']['f1'] * 100))

                # 统计23标签的总体结果：
                metrics_23 = metric_23_class(gt, predict)
                print(f"\n23标签的总体测试结果: \n")
                print("Micro: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    metrics_23['micro']['precision'] * 100,
                    metrics_23['micro']['recall'] * 100,
                    metrics_23['micro']['f1'] * 100))
                print("Macro: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    metrics_23['macro']['precision'] * 100,
                    metrics_23['macro']['recall'] * 100,
                    metrics_23['macro']['f1'] * 100))
                print("Weighted: Precision: {:.2f}, Recall: {:.2f}, F1: {:.2f}".format(
                    metrics_23['weighted']['precision'] * 100,
