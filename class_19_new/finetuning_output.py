...
# 计算每个标签的权重
label_counts = np.array(train_labels).sum(axis=0)
total_counts = len(train_labels)
pos_weights = (total_counts - label_counts) / label_counts
...
# 将控制台输出内容保存为txt文件
output_file = "output_finetuning_sigmoid.txt"
                ...
                for epoch in range(num_epochs):
                    ...
                    for batch in val_loader:
                        ...
                        with torch.no_grad():
                            ...
                            # print("eval_logits: ", logits_np)

                            # 现在的操作：将 logits 转换为概率
                            logits_sigmoid = torch.sigmoid(logits)
                            # 将 logits 转换为 CPU 的 NumPy 数组
